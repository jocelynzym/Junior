...
#读csv文件,创建实验数据
dataset1 = pd.read_csv('./DEAP/EEG_feature.csv',header=None)
dataset2 = pd.read_csv('./DEAP/valence_arousal_label.csv',header=None)
dataset3 = pd.read_csv('./DEAP/subject_video.csv',header=None)
...
#将3个文件按样本序号合并
x_y = np.concatenate([z_raw,x_,y_raw],axis=1)
print("shape",x_y[0])
...
#将样本均匀划分为5等分
for i in range(0,32):#32个用户
    count = 0
    for j in range(0,38):#每个用户38条记录
        index = i*38 + j
        count += 1
        # 写入csv文件
        if count == 1:
...
df_test1.to_csv("./DEAP/sample_1.csv",  header=False, index=False, sep=',')
df_test2 = pd.DataFrame(mat2)
df_test2.to_csv("./DEAP/sample_2.csv",  header=False, index=False, sep=',')
df_test3 = pd.DataFrame(mat3)
df_test3.to_csv("./DEAP/sample_3.csv",  header=False, index=False, sep=',')
df_test4 = pd.DataFrame(mat4)
df_test4.to_csv("./DEAP/sample_4.csv",  header=False, index=False, sep=',')
df_test5 = pd.DataFrame(mat5)
df_test5.to_csv("./DEAP/sample_5.csv",  header=False, index=False, sep=',')
